...
本目录的 `genshen-cp12.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp12.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp12 deepking          # 导出 ~/.genshen-cp12/deepking/*.skin.json
  ```
...
```bash
genshen-cp12 deepking --check    # 应输出「通过」
genshen-cp12 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp12-light.jpg` —— 取「晴空」那张的天台双人, 配浅色界面
- `mascot-cp12-dark.jpg` —— 取「月下」那张的满月构图, 配深色界面
