...
本套件有三张素材(行秋、重云), 每张三种摆法, 共 9 种壁纸:

  genshen-cp12                 # 默认: single1 卡片式(模糊背景 + 居中圆角卡片)
  genshen-cp12 cover           # 满屏: cover 裁切铺满整屏, 无边框
  genshen-cp12 showall         # 完整: 等比放进纯色底, 一个像素都不裁
  genshen-cp12 1               # 等价于 single1(兼容按张编号的写法)
  genshen-cp12 list            # 列出所有可切换样式
  genshen-cp12 random          # 随机来一张
  genshen-cp12 all             # 生成全部样式到 ~/.genshen-cp12/wallpapers
  genshen-cp12 all --out DIR   # 生成到指定目录(JetBrains 背景图用)
  genshen-cp12 cycle 30        # 每 30 分钟自动随机换壁纸
  genshen-cp12 switcher        # 打开可视化切换器
  genshen-cp12 pet             # 启动桌面桌宠
  genshen-cp12 deepking        # 生成 DeepKing 界面皮肤 + 离线预览
  genshen-cp12 copy            # 只合成不设置
...
def build_parser():
    ...
    # 每一种模式都注册成子命令。
    # argparse 的子命令是"贪婪"的: 一旦注册了子解析器, `genshen-cp12 cover`
    ...
    # 覆盖掉子解析器 set_defaults 设的 mode, 于是 `genshen-cp12 cover2` 反而
