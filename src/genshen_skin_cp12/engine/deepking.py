...
def pkg_root():
    """包目录 .../genshen_skin_cp12(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp12.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #e9f2fa;
  --bg-layer-3: #d5e6f5;
  --label-primary: #16233a;
  --label-secondary: #3f5474;
  --label-tertiary: #748aa8;
  --brand-primary: #2f6bab;
  --accent: #2f6bab;
  --primary: #2f6bab;
  --interactive-bg-hover-solid: #dcebf7;
  --interactive-bg-active: #c3dcef;
  --border-l1: #d6e6f3;
  --border-l2: #bcd4e8;
  --border: #bcd4e8;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP12</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP12", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP12", mascot.get("dark")),
    }
